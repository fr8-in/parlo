...
const FilterTruckType = (props: Props) => {
    const { onSelect, selected } = props
    const { data, error, isValidating, mutate } = useFrappeGetDocList<TruckType>(
        "Truck Type", {
        fields: ["name"],
    })
    ...
    return (
        <>
            {(data && data?.length > 0) ? data.map((user: TruckType, i: number) => {
...
export default FilterTruckType
