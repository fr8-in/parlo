...
const TripTable = (props: Props) => {
    const { dataSource, hide_filters, handleSorter, handleSearch, rowSelection, sorter, search, is_payment_tab, tabKey } = props;
    ...
    const columns: any = [
        ...(hide_filters ? [] : [{
            ...
            render: (record: any) => {
                return (
                    <Space>
                        <IconButton size='small' onClick={() => onPressCall(record?.cell_number)}>
                            <LocalPhoneIcon color='secondary' fontSize='small' />
                        </IconButton>
                         <IconButton color='secondary' size='small' disabled={!!record.invoiced_at} onClick={() => handleShow('showCharges', '', 'tripName', record.name)}>
        ...
        }]),
        {
            title: "ID",
            render: (record: any) => {
                return <Button size='small' color='secondary' onClick={() => navigate(`/trip/${record?.name}`)}>{record?.id}</Button>
            },
            width: "7%",
    ...
    const hide_rowselection = (hide_filters || tabKey === 'All' || tabKey === 'Active')
