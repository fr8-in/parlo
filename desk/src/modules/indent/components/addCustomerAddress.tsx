 ...
 * @returns state based Add Customer Address from
 */

const AddCustomerAddress = (props: Props) => {
    const { onClose, callBack, name, mutate, customer } = props
    ...
    const onSubmit = async () => {
        if ((!form.address || !form.pan || !form.gst || !form.short_name)) {
            ...
            try {
                await createDoc('Customer Address', { ...form, customer })
                callBack(form)
                mutate()
            } catch (error: any) {
    ...
    return (
        <div className="p-3 pt-0 gap-3 h-full">
            <div className="flex justify-between items-center h-16 gap-3 mb-4">
                <h5 className="font-bold">Add Customer Address</h5>
