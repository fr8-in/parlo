...
const AddConsigneeLocation = (props: AddCustomerProps) => {
    const { onClose, consignee, callBack,consigneeLocation } = props
    const initialForm = {
        name: null,
        city: null
    }
    ...
    useEffect(() => {
        setValue('name', consigneeLocation)
    }, [consigneeLocation])
    ...
    const { call, error, loading } = useFrappePostCall('parlo.trip.addConsigneeLocation.get_or_add_consignee_location')
    ...
    const onSubmit = async (value: any) => {
        console.log({ value });
        const consigneeLocationInput = {
            ...value,
            consignee
        }

        await call({ addConsigneeLocationInput: consigneeLocationInput }).then(
            (result: any) => {
                console.log({ result: result?.message })
                message.success('Consignee Location added successfully')
            ...
            if (httpStatusText == 'CONFLICT') {
                message.error('Delivery Location already exists')
            } else {
    ...
    return (
        <div className="p-3 pt-0 gap-3 h-full">
            <div className="flex justify-between items-center h-16 gap-3 mb-4">
                <h5 className="font-bold">Add Consignee Location</h5>
...
export default AddConsigneeLocation;