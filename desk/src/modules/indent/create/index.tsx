...
const CreateIndent = (props: Props) => {
    ...
    console.log('indentData :', indentData);
        ...
        const updatedItem = !isEmpty(items) ?  items.map((item:MaterialType) => {
            return {
              ...item,
    ...
    const handleCustomer = (value: any) => {
        ...
        handleConsignee(initialForm.consignee);
        clearErrors();
    };
    ...
    const handleMaterialSum = (materials: Array<MaterialType> , customer_price?:number) => {
    ...
    const handleEditMaterialType = (material: MaterialType) => {
        const removeMaterial = state.materials.filter((m: MaterialType) => m.id !== material.id)
        setState((prev: any) => {
            return (
    ...
    const handleDeleteMaterialType = (material: MaterialType) => {
        const removeMaterial = state.materials.filter((m: MaterialType) => m.id !== material.id)
        setState((prev: any) => {
            return (
    ...
    const handleRateType = (value: TruckType) => {
        setCalculatablesToInitial(initialForm).forEach(({name,value}) => (setValue(name , value)))
    ...
    const onSubmit = async (form: FormProp) => {
        ...
        console.log("INDENT INPUTS ", indentInput)
        if(isIndentEdit){
            console.log("EDIT INDENT VALUES", form );
