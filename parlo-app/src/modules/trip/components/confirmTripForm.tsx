...
import { FormProp, initialForm, initial_advance } from "./initialForm";
import { InputController } from "../../../common/form/InputController";
import FuelAdvance from "./fuelAdvance";
...
import { setConfirmTripForm } from "./formSetters";

interface ConfirmTripFormProps {
    indentNames : Array<string>;
    tripData?:any
    fromAssignedTrip?:boolean;
    tripName?:any
    assignTrip?:boolean
}
const ConfirmTripForm = (props:ConfirmTripFormProps) => {
    const theme = useTheme();
    const isMobile = useMediaQuery(theme.breakpoints.down("md"));
    const { indentNames , tripData , fromAssignedTrip , tripName , assignTrip} = props 
    ...
    const { call: assignTripCall , error:assignTripError } = useFrappePostCall('parlo.trip.assignTrip.assignTrip')
    ...
    const prefillDataFromTripData = (tripData:any) => {
        console.log("PREFILL", tripData)
        setConfirmTripForm(tripData).forEach(({name,value}) => (setValue(name , value)))
    }
    ...
    useEffect(() => {
        if (data) {
            ...
            if(fromAssignedTrip){
                prefillDataFromTripData(tripData)
            }else{
    ...
    console.log("DATA FROM FORM", data , indentNames , tripData )
    ...
    const onSubmit = async (form:FormProp) => {
        ...
        try {
            await call({ confirmTripInput }).then((response: any) => {
                ...
                navigate(`/trip/${response?.message}`,{replace:true})
            })
    ...
    /**
     * @author Prasanth.M
     * @function onClickAssignLoad
     * @description This function is used to move indents to assigned.User can assign indents and then confirm it later.
     */
    const onClickAssignLoad = async () => {
        const input = getValues()
        const assignTripInput = {...input , tripName:tripName , isTripUpdate: tripName ? true : false , indents: indent.indent_names }
           await assignTripCall({ assignTripInput }).then((
                response: any
            ) => {
                console.log("ASSIGN LOAD RESPONE ",JSON.stringify(response))
                message.success(`Load Assigned Successfully`)
                navigate('/',{ replace : true })
            }).catch(error => {
                console.log("ASSIGN LOAD RESPONE ",JSON.stringify(error))
                const exception = error?.exception
                if (`${exception}`.includes('Driver')) {
                    message.error("Driver in active trip")
                } else if (`${exception}`.includes('Truck')) {
    ...
    return (
        <div className="max-w-3xl mx-auto relative" style={{ height: 'calc(100vh - 90px)' }}>
            <Card sx={{ mb: 1 }}>
                ...
                <Button color="secondary" onClick={onClickAssignLoad} hidden={ !assignTrip } variant="contained" size="small" sx={{height:30, marginRight:2}}  >
                    Assign
                </Button>
                </div>
            </Card>
